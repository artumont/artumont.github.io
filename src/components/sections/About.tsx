...
const profileButtons = [
  ...
  { name: "Email", icon: <Mail />, href: "mailto:artumontg@gmail.com" },
  {
    name: "Linkedin",
    icon: <Linkedin />,
    href: "https://www.linkedin.com/in/artumont",
  },
  {
    name: "Leetcode",
    icon: (
    ...
    href: "https://leetcode.com/u/artumont/",
  },
];
      ...
      <div className="relative h-full w-full bg-primary px-5 py-12 lg:px-32">
        ...
        >
          <div className="flex flex-col lg:max-w-[50%] bg-secondary mt-10 rounded-lg p-5 mx-5 lg:mx-0 lg:mr-5">
            ...
            <div className="flex flex-col mt-2">
              {traits.map((trait, index) => (
                <div
                  key={index}
                  className="rounded-lg bg-terciary p-2 text-center gap-1 m-1"
                >
          ...
          <div className="flex flex-col min-w-[47%] bg-secondary mt-10 rounded-lg p-5 items-center mx-5 lg:mx-0">
            <h1 className="text-xl font-jetbrains mb-1 text-center">
              Technical Expertise
            </h1>
          ...
          <div className="flex flex-col bg-secondary mt-5 rounded-lg p-5 items-center mx-5 lg:mx-0 w-full">
            ...
            <div className="flex flex-col h-full w-full mt-6">
              {workExperience.map((work, index) => (
                <div
                  key={index}
                  className="flex flex-col lg:flex-row items-center lg:items-start bg-terciary p-4 gap-1 m-1 rounded-lg"
                >
                  <Image
                    src={work.logo}
                    alt={work.company}
                  ...
                  <div className="flex flex-col lg:ml-5 text-center lg:text-left">
                    ...
                    <h2 className="text-md font-jetbrains text-accent">
                      {" "}
                      {work.role}{" "}
                    </h2>
                    <h3 className="text-sm font-inter text-accent italic">
                      {" "}
                      {work.duration}{" "}
                    </h3>
                    <p className="text-md font-inter text-accent mt-2 text-justify">
                      {" "}
                      {work.description}{" "}
                    </p>
                  </div>
          ...
          <div className="flex flex-col bg-secondary mt-5 rounded-lg p-5 items-center mx-5 lg:mx-0 w-full">
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-6">
              {certifications.map((cert, i) => (
                <div
                  key={i}
                  className="flex flex-col bg-terciary p-3 rounded-lg min-w-[200px]"
                >
                  <span className="text-sm font-jetbrains text-accent">
                    {cert.issuer}
                  </span>
                  <span className="text-sm font-inter">{cert.name}</span>
                  <span className="text-xs font-inter text-accent/70">
                    {cert.date}
                  </span>
                </div>
              ))}
